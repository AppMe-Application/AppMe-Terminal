...
public class TerminalDebianView extends ViewFlipper implements Iterable<View> {
    ...
    public TerminalDebianView(Context context) {
        super(context);
        commonConstructor(context);
    }

    public TerminalDebianView(Context context, AttributeSet attrs) {
        super(context, attrs);
        commonConstructor(context);
    }
    ...
    private void showTitle() {
        ...
        String title = context.getString(R.string.terminal_window_title, getDisplayedChild()+1);
        StringBuilder sb = new StringBuilder();
        sb.append("Debian").append(" ");
        sb.append(title);
        
        if (session instanceof GenericTermSession) {
            title = ((GenericTermSession) session).getTitle(sb.toString());
        }      

        if (mToast == null) {
            mToast = Toast.makeText(context, sb.toString(), Toast.LENGTH_SHORT);
